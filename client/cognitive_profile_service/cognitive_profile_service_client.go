...
type ClientService interface {
	...
	SearchCognitiveProfileVoices(params *SearchCognitiveProfileVoicesParams, opts ...ClientOption) (*SearchCognitiveProfileVoicesOK, error)
...
/*
SearchCognitiveProfileVoices searches voices
*/

func (a *Client) SearchCognitiveProfileVoices(params *SearchCognitiveProfileVoicesParams, opts ...ClientOption) (*SearchCognitiveProfileVoicesOK, error) {
	if params == nil {
		params = NewSearchCognitiveProfileVoicesParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "SearchCognitiveProfileVoices",
		Method:             "GET",
		PathPattern:        "/storage/cognitive_profiles/{id}/voice",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &SearchCognitiveProfileVoicesReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	for _, opt := range opts {
	...
	success, ok := result.(*SearchCognitiveProfileVoicesOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for SearchCognitiveProfileVoices: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
